...
def mainlist(item):
    ...
    itemlist.append( Item(channel=__channel__, action="series"       , title="Últimos agregados"    , url="http://reyanime.com/animes/ultimos/" ))
    itemlist.append( Item(channel=__channel__, action="series"       , title="Proximamente"         , url="http://reyanime.com/animes/proximamente/" ))
...
def letras(item):
    ...
    data = scrapertools.find_single_match(data,'<div class="top-list-anim">Alfab[^<]+</div>(.*?)</div>')
    patron = '<a href="([^"]+)[^>]+>([^<]+)</a>'
...
def generos(item):
    ...
    data = scrapertools.cache_page(item.url)
    data = scrapertools.get_match(data,'<div class="top-list-anim">G[^<]+</div>(.*?)</div>\s')
    logger.info("data="+data)
    patron = '<a href="([^"]+)"><div[^<]+</div>([^<]+)</a>'
    matches = re.compile(patron,re.DOTALL).findall(data)    
...
def series(item):
    ...
    patron  = '(<a href="[^"]+"[^<]+'
    patron += '<div class="anim-list">.*?</div[^<]+</div[^<]+</a)'
    ...
    for match in matches:
        ...
        generos = scrapertools.find_single_match(match,'<div class="anim-list-genr-iz">([^<]+)<')
        scrapedplot = scrapertools.find_single_match(match,'<div class="line-title"></div>([^<]+)<')
        scrapedthumbnail = scrapertools.find_single_match(match,'src="([^"]+)"')
        scrapedtitle = scrapertools.find_single_match(match,'<div onmousemove[^>]+>([^<]+)<')
    ...
    next_page = scrapertools.find_single_match(data,'<a href="([^"]+)" class="next">next .rsaquo')
    if next_page!="":
...
def episodios(item):
    ...
    # Descarga la pagina
    data = scrapertools.cache_page(item.url)
    data = scrapertools.find_single_match(data,'<div id="lcmain"(.*?)</ul>')
    ...
    for scrapedurl,scrapedtitle in matches:
        title = scrapedtitle.strip()
        try:
            episodio = scrapertools.get_match(scrapedtitle,"Capitulo\s+(\d+)")
            if len(episodio)==1:
                title = "1x0"+episodio
            else:
                title = "1x"+episodio
...
def findvideos(item):
    ...
    patron = '<div class="tabitem"[^<]+<iframe src="(http://ypfserviclubs.org/repro-rc/[a-z0-9]+\?v\=[^"]+)"'
    matches = re.compile(patron,re.DOTALL).findall(data)
    if (DEBUG): scrapertools.printMatches(matches)

    for page_url in matches:
        logger.info("page_url="+page_url)
        servercode = scrapertools.find_single_match(page_url,'http://ypfserviclubs.org/repro-rc/([a-z0-9]+)')
